...
export class Mail {
  ...
  constructor(private _config: ISmtpConfig) {
    ...
    if (!_smtp) this._config.smtp = { disabled: true }

    if (_smtp.disabled === true) return // don't analyze config if disabled
    ...
    if (!this._authTypes.includes(_smtp.type.trim().toLowerCase()))
      throw new Error(
        `[smtp.type] Authentication type not found ${_smtp.type.trim().toLowerCase()}`
      )

    _smtp.type = _smtp.type.trim().toLowerCase()

    if (!_smtp.host) throw new Error(`[smtp.host] not set`)
    if (!_smtp.port) throw new Error(`[smtp.port] not set`)

    try {
      this._template = readFileSync("template.html", "utf8")
    } catch {
  ...
  async send(message: IMessage) {
    ...
    const temp = {
      ...
      auth: null,
      name:
        typeof this._config.smtp.clientHostName === "string" && this._config.smtp.clientHostName
          ? this._config.smtp.clientHostName
          : null
    }
    ...
    const transport = createTransport(temp),
      headers = {}
    ...
    await transport.sendMail({
      ...
      html: this._template
        ...
        .replace(/<!--\s*timeStamp\s*-->/, new Date().toLocaleString("en-AU", { hour12: false })),
