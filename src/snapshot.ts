...
export class Snapshot {
  ...
  constructor(private _config: IShapshotConfig) {
    if (!this._config.snapshot) this._config.snapshot = {}
  ...
  push(appId: number, app: string, key: string, history: boolean, v: IValue) {
    if (!this._data.app[appId]) this._data.app[appId] = { name: app, metric: {}, inactive: false }
  ...
  last(appId: number, key: string) {
    if (!this._data.app[appId] || !this._data.app[appId].metric[key]) return undefined
  ...
  dump() {
    ...
    writeFile(`History_${new Date().toISOString()}.json`, JSON.stringify(this._data), (ex) => {
      if (ex) error(`can't dump history -> ${ex.message || ex}`)
    })
  }

  async send() {
    if (
      !this._config.snapshot.url ||
      !this._config.snapshot.token ||
      this._config.snapshot.disabled === true
    )
      return
  ...
  inactivate() {
    ...
    for (const id of Object.keys(this._data.app)) {
      const app = this._data.app[<any>id],
        dt = (t - app.timeStamp) / 60000

      const inactive = dt > this._config.snapshot.inactiveAfterM
      if (app.inactive !== inactive) info(`app [${app.name}] inactive = ${inactive}`)
